...
use CController, CControllerResponseData, CControllerResponseFatal, CControllerResponseRedirect, CRoleHelper, CSystemInfoHelper, CZabbixServer, CSettingsHelper, CSessionHelper, API, CArrayHelper, CUrl;
...
class InspectionDetail extends CController {
    ...
    /**
     * Returns the max trend values of the given item(s) for the quarter that
     * corresponds to the selected inspection cycle (0=Q1 .. 3=Q4).
     */
    private function getTrendMaxForCycle($itemids, int $cycle): array {
        $ranges = [
            0 => ['-01-01', '-03-31'],
            1 => ['-04-01', '-06-30'],
            2 => ['-07-01', '-09-30'],
        ...
        list($from, $till) = array_key_exists($cycle, $ranges) ? $ranges[$cycle] : $ranges[0];

        return API::Trend()->get([
            'output' => ['value_max'],
            'itemids' => $itemids,
            'time_from' => strtotime(date('Y') . $from),
            'time_till' => strtotime(date('Y') . $till)
        ]);
    }

    /**
     * Masks sensitive directives (passwords, tokens, TLS material) before the
     * raw configuration file is rendered in the report or kept in $data.
     */
    private function scrubSecrets($conf): string {
        if (!is_string($conf) || $conf === '') {
            return (string) $conf;
        }

        return preg_replace(
            '/^(\s*(?:DBPassword|DBUser|TLSPSKIdentity|TLSPSKFile|VaultToken|VaultDBPath|DBTLSKeyFile|DBTLSCertFile)\s*=).*$/mi',
            '$1 ********',
            $conf
        );
    }
    ...
    /**
     * Returns the latest stored value for $key on $hostid, auto-detecting the
     * item value type so it works for character, text or numeric items.
     */
    private function getLatestValue($hostid, string $key): string {
        $item = API::Item()->get([
        ...
        $history = API::History()->get([
            'output' => ['value'],
            'itemids' => ($item[0]['itemid'] ?? 0),
            'history' => (int) ($item[0]['value_type'] ?? 4),
            'sortfield' => 'clock',
            'sortorder' => 'DESC',
            'limit' => 1
        ]);

        return (string) ($history[0]['value'] ?? '');
    }

    /**
     * Detects whether the given host runs inside a container (Docker, Podman,
     * containerd, Kubernetes). Detection order:
     *   1) explicit runtime item reported by the agent (zabbix.<role>.runtime);
     *   2) presence of a Docker template / docker.* items on the host;
     *   3) container hints in system.uname / system.sw.os.
     */
    private function detectDocker($hostid, string $runtime_key): bool {
        $runtime = strtolower($this->getLatestValue($hostid, $runtime_key));
        if ($runtime !== '') {
            return (strpos($runtime, 'docker') !== false
                || strpos($runtime, 'container') !== false
                || strpos($runtime, 'kube') !== false
                || strpos($runtime, 'podman') !== false);
        }
        ...
        foreach (($host[0]['parentTemplates'] ?? []) as $tpl) {
            if (stripos($tpl['name'], 'docker') !== false || stripos($tpl['name'], 'container') !== false) {
                return true;
            }
        }

        foreach (['system.uname', 'system.sw.os'] as $key) {
            $val = strtolower($this->getLatestValue($hostid, $key));
            if ($val !== '' && (strpos($val, 'docker') !== false || strpos($val, 'container') !== false)) {
                return true;
            }
        }
    ...
    protected function doAction(): void {
        ...
        if ($data['host_sel'] != -1) {
            $this->setResponse(new CControllerResponseRedirect(
                (new CUrl('zabbix.php'))
                    ->setArgument('action', 'inspection.report')
                    ->setArgument('sel', $data['host_sel'])
            ));
        ...
        if ($data['host_sel'] == -1) {
            ...
            $zabbix_server_info = [
                ...
                'zabbix_server_runtime' => 'bare',
                'zabbix_server_conf_source' => 'file'
            ];
            ...
            foreach ($zabbix_server_ids[0] as $zabbix_server_id) {
                ...
                $zabbix_server_cpu_num = API::History()->get([
                    'output' => ['value', 'clock'],
                    'itemids' => ($zabbix_server_itemid[0] ?? 0),
                    'history' => 3,
                    'sortfield' => 'clock',
                    'sortorder' => 'DESC',
                    'limit' => 1
                ...
                $zabbix_server_info['zabbix_server_mem_total_size'] = round(((float) ($zabbix_server_mem_total_size[0]['value'] ?? 0))/1024/1024/1024,2);
                ...
                $zabbix_server_info['zabbix_server_conf'] = $this->scrubSecrets($zabbix_server_conf[0]['value'] ?? '');
                ...
                $zabbix_server_info['zabbix_server_conf_source'] = 'file';
                $zabbix_server_info['zabbix_server_runtime'] = $this->detectDocker($zabbix_server_id, 'zabbix.server.runtime') ? 'docker' : 'bare';

                if ($zabbix_server_info['zabbix_server_runtime'] === 'docker') {
                    $zabbix_server_env = $this->scrubSecrets($this->getLatestValue($zabbix_server_id, 'zabbix.server.env'));
                    if (trim($zabbix_server_env) !== '') {
                        $zabbix_server_info['zabbix_server_conf'] = $zabbix_server_env;
                        $zabbix_server_info['zabbix_server_conf_source'] = 'env';
                    }

                    $result_analysis['description'] = 'Containerized Zabbix server detected.';
                    $result_analysis['type'] = 'docker';
                    $result_analysis['value'] = $zabbix_server_info['zabbix_server_hostname'];
                    $result_analysis['threshold'] = '';
                    $result_analysis['analysis'] = 'This Zabbix server runs inside a container. Tuning is provided through ZBX_* environment variables instead of a flat zabbix_server.conf, and logs are written to the container stdout.';
                    $result_analysis['suggestion'] = 'Set CPU/memory limits and a restart policy on the container; tune cache/process parameters via ZBX_* env vars; collect logs through the container log driver (docker logs / journald).';
            ...
            $zabbix_proxy_info = [
                ...
                'zabbix_proxy_state' => '',
                'zabbix_proxy_nvps' => 0,
                'zabbix_proxy_runtime' => 'bare',
                'zabbix_proxy_conf_source' => 'file'
            ];
            ...
            if ($this->hasInput('zabbix_proxy_ids', [])) {
                foreach ($zabbix_proxy_ids[0] as $zabbix_proxy_id) {
                    ...
                    $zabbix_proxy_cpu_num = API::History()->get([
                        'output' => ['value', 'clock'],
                        'itemids' => ($zabbix_proxy_itemid[0] ?? 0),
                        'history' => 3,
                        'sortfield' => 'clock',
                        'sortorder' => 'DESC',
                        'limit' => 1
                    ...
                    $zabbix_proxy_info['zabbix_proxy_mem_total_size'] = round(((float) ($zabbix_proxy_mem_total_size[0]['value'] ?? 0))/1024/1024/1024,2);
                    ...
                    $zabbix_proxy_info['zabbix_proxy_conf'] = $this->scrubSecrets($zabbix_proxy_conf[0]['value'] ?? '');
                    ...
                    if (($zabbix_proxy_state[0]['operating_mode'] ?? 0) == 0) {
                        $zabbix_proxy_info['zabbix_proxy_mode'] = 'Active';
                    }
                    elseif (($zabbix_proxy_state[0]['operating_mode'] ?? 0) == 1) {
                        $zabbix_proxy_info['zabbix_proxy_mode'] = 'Passive';
                    }
                    if (($zabbix_proxy_state[0]['state'] ?? 0) == 0) {
                        $zabbix_proxy_info['zabbix_proxy_state'] = 'Unknown';
                        $zbx_proxy_state = 1;
                    }
                    elseif (($zabbix_proxy_state[0]['state'] ?? 0) == 1) {
                        $zabbix_proxy_info['zabbix_proxy_state'] = 'Offline';
                        $zbx_proxy_state = 2;
                    }
                    elseif (($zabbix_proxy_state[0]['state'] ?? 0) == 2) {
                    ...
                    if ($zabbix_proxy_nvps != []) {
                        $zabbix_proxy_info['zabbix_proxy_nvps'] = round(((float) ($zabbix_proxy_nvps[0]['value'] ?? 0)), 2);
                    }

                    $zabbix_proxy_info['zabbix_proxy_conf_source'] = 'file';
                    $zabbix_proxy_info['zabbix_proxy_runtime'] = $this->detectDocker($zabbix_proxy_id, 'zabbix.proxy.runtime') ? 'docker' : 'bare';

                    if ($zabbix_proxy_info['zabbix_proxy_runtime'] === 'docker') {
                        $zabbix_proxy_env = $this->scrubSecrets($this->getLatestValue($zabbix_proxy_id, 'zabbix.proxy.env'));
                        if (trim($zabbix_proxy_env) !== '') {
                            $zabbix_proxy_info['zabbix_proxy_conf'] = $zabbix_proxy_env;
                            $zabbix_proxy_info['zabbix_proxy_conf_source'] = 'env';
                        }

                        $result_analysis['description'] = 'Containerized Zabbix proxy detected.';
                        $result_analysis['type'] = 'docker';
                        $result_analysis['value'] = $zabbix_proxy_info['zabbix_proxy_hostname'];
                        $result_analysis['threshold'] = '';
                        $result_analysis['analysis'] = 'This Zabbix proxy runs inside a container. Tuning is provided through ZBX_* environment variables instead of a flat zabbix_proxy.conf, and logs are written to the container stdout.';
                        $result_analysis['suggestion'] = 'Set CPU/memory limits and a restart policy on the container; size ConfigFrequency/DataSenderFrequency and cache parameters via ZBX_* env vars; collect logs through the container log driver (docker logs / journald).';
                        array_push($data['result_analysis'], $result_analysis);
                    }
            ...
            foreach ($zabbix_database_ids[0] as $zabbix_database_id) {
                ...
                $zabbix_database_cpu_num = API::History()->get([
                    'output' => ['value', 'clock'],
                    'itemids' => ($zabbix_database_itemid[0] ?? 0),
                    'history' => 3,
                    'sortfield' => 'clock',
                    'sortorder' => 'DESC',
                    'limit' => 1
                ...
                $zabbix_database_info['zabbix_database_mem_total_size'] = round(((float) ($zabbix_database_mem_total_size[0]['value'] ?? 0))/1024/1024/1024,2);
                ...
                if (stristr(($zabbix_database_dbver[0]['value'] ?? ''), 'mysql') || stristr(($zabbix_database_dbver[0]['value'] ?? ''), 'mariadb')) {
                    ...
                    $zabbix_database_bpu = $this->getTrendMaxForCycle(($zabbix_database_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zabbix_database_info['zabbix_database_bpu'] = round((float) (max($zabbix_database_bpu)['value_max'] ?? 0), 2);
                    if (round((float) (max($zabbix_database_bpu)['value_max'] ?? 0), 2) < 80) {
                        $mysql_buffer_pool_util = round((float) (max($zabbix_database_bpu)['value_max'] ?? 0), 2);
                    }
                    ...
                    $zabbix_database_cir = $this->getTrendMaxForCycle(($zabbix_database_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zabbix_database_info['zabbix_database_cir'] = round((float) (max($zabbix_database_cir)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_database_csr = $this->getTrendMaxForCycle(($zabbix_database_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zabbix_database_info['zabbix_database_csr'] = round((float) (max($zabbix_database_csr)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_database_cps = $this->getTrendMaxForCycle(($zabbix_database_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zabbix_database_info['zabbix_database_cps'] = round((float) (max($zabbix_database_cps)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_database_used_conn = $this->getTrendMaxForCycle(($zabbix_database_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zabbix_database_info['zabbix_database_used_conn'] = round((float) (max($zabbix_database_used_conn)['value_max'] ?? 0), 2);
                ...
                $zabbix_database_info['zabbix_database_runtime'] = $this->detectDocker($zabbix_database_id, 'zabbix.database.runtime') ? 'docker' : 'bare';

                if ($zabbix_database_info['zabbix_database_runtime'] === 'docker') {
                    $result_analysis['description'] = 'Containerized Zabbix database detected.';
                    $result_analysis['type'] = 'docker';
                    $result_analysis['value'] = $zabbix_database_info['zabbix_database_hostname'];
                    $result_analysis['threshold'] = '';
                    $result_analysis['analysis'] = 'This database runs inside a container. Buffer pool and connection limits depend on the container resources and the MYSQL_*/MARIADB_* environment configuration.';
                    $result_analysis['suggestion'] = 'Set CPU/memory limits and a restart policy; persist the data directory on a named volume; size innodb_buffer_pool_size to the container memory limit.';
                    array_push($data['result_analysis'], $result_analysis);
                }
            ...
            $zabbix_server_itemid = API::Item()->get([
                'output' => ['itemid'],
                'hostids' => ($zabbix_server_hostid[0]['hostid'] ?? 0),
                'search' => [
            ...
            if (!is_array($queue_overview_data)) {
                $queue_overview_data = [];
            }
            ...
            foreach ($zabbix_server_ids[0] as $zabbix_server_id) {
                ...
                $zabbix_server_cpu_util = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                $zbx_server_perf['cpu_util'] = round((float) (max($zabbix_server_cpu_util)['value_max'] ?? 0), 2);
                ...
                $zabbix_server_cpu_iowait = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                $zbx_server_perf['cpu_iowait'] = round((float) (max($zabbix_server_cpu_iowait)['value_max'] ?? 0), 2);
                ...
                $zabbix_server_cpu_load = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                $zbx_server_perf['cpu_load'] = round((float) (max($zabbix_server_cpu_load)['value_max'] ?? 0), 2);
                ...
                $zabbix_server_mem_util = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                $zbx_server_perf['mem_util'] = round((float) (max($zabbix_server_mem_util)['value_max'] ?? 0), 2);
                ...
                $zabbix_server_swap_util = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                $zbx_server_perf['swap_util'] = round(100 - (float) (max($zabbix_server_swap_util)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_rcache_buffer_pused = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_rcache_buffer_pused != []) {
                $zbx_server_cache['rcache_buffer_pused'] = round((float) (max($zabbix_server_rcache_buffer_pused)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_wcache_index_pused = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_wcache_index_pused != []) {
                $zbx_server_cache['wcache_index_pused'] = round((float) (max($zabbix_server_wcache_index_pused)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_wcache_history_pused = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_wcache_history_pused != []) {
                $zbx_server_cache['wcache_history_pused'] = round((float) (max($zabbix_server_wcache_history_pused)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_tcache_pmisses = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_tcache_pmisses != []) {
                $zbx_server_cache['tcache_pmisses'] = round((float) (max($zabbix_server_tcache_pmisses)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_tcache_pitems = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_tcache_pitems != []) {
                $zbx_server_cache['tcache_pitems'] = round((float) (max($zabbix_server_tcache_pitems)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_wcache_trend_pused = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_wcache_trend_pused != []) {
                $zbx_server_cache['wcache_trend_pused'] = round((float) (max($zabbix_server_wcache_trend_pused)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_vcache_buffer_pused = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_vcache_buffer_pused != []) {
                $zbx_server_cache['vcache_buffer_pused'] = round((float) (max($zabbix_server_vcache_buffer_pused)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_vcache_cache_hits = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_vcache_cache_hits != []) {
                $zbx_server_cache['vcache_cache_hits'] = round((float) (max($zabbix_server_vcache_cache_hits)['value_max'] ?? 0), 2);
            }
            ...
            $zabbix_server_vcache_cache_misses = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_vcache_cache_misses != []) {
                $zbx_server_cache['vcache_cache_misses'] = round((float) (max($zabbix_server_vcache_cache_misses)['value_max'] ?? 0), 2);
            }
            ...
            $zabbix_server_vmware_buffer_pused = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
            
            if ($zabbix_server_vmware_buffer_pused != []) {
                $zbx_server_cache['vmware_buffer_pused'] = round((float) (max($zabbix_server_vmware_buffer_pused)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_alerter_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_alerter_avg_busy != []) {
                $zbx_server_internal_processes['process_alerter_avg_busy'] = round((float) (max($zabbix_server_process_alerter_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_alert_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_alert_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_alert_manager_avg_busy'] = round((float) (max($zabbix_server_process_alert_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_alert_syncer_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_alert_syncer_avg_busy != []) {
                $zbx_server_internal_processes['process_alert_syncer_avg_busy'] = round((float) (max($zabbix_server_process_alert_syncer_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_availability_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_availability_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_availability_manager_avg_busy'] = round((float) (max($zabbix_server_process_availability_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_configuration_syncer_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_configuration_syncer_avg_busy != []) {
                $zbx_server_internal_processes['process_configuration_syncer_avg_busy'] = round((float) (max($zabbix_server_process_configuration_syncer_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_configuration_syncer_worker_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_configuration_syncer_worker_avg_busy != []) {
                $zbx_server_internal_processes['process_configuration_syncer_worker_avg_busy'] = round((float) (max($zabbix_server_process_configuration_syncer_worker_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_connector_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_connector_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_connector_manager_avg_busy'] = round((float) (max($zabbix_server_process_connector_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_connector_worker_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_connector_worker_avg_busy != []) {
                $zbx_server_internal_processes['process_connector_worker_avg_busy'] = round((float) (max($zabbix_server_process_connector_worker_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_discovery_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_discovery_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_discovery_manager_avg_busy'] = round((float) (max($zabbix_server_process_discovery_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_discovery_worker_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_discovery_worker_avg_busy != []) {
                $zbx_server_internal_processes['process_discovery_worker_avg_busy'] = round((float) (max($zabbix_server_process_discovery_worker_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_escalator_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_escalator_avg_busy != []) {
                $zbx_server_internal_processes['process_escalator_avg_busy'] = round((float) (max($zabbix_server_process_escalator_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_history_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_history_poller_avg_busy != []) {
                $zbx_server_internal_processes['process_history_poller_avg_busy'] = round((float) (max($zabbix_server_process_history_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_history_syncer_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_history_syncer_avg_busy != []) {
                $zbx_server_internal_processes['process_history_syncer_avg_busy'] = round((float) (max($zabbix_server_process_history_syncer_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_housekeeper_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_housekeeper_avg_busy != []) {
                $zbx_server_internal_processes['process_housekeeper_avg_busy'] = round((float) (max($zabbix_server_process_housekeeper_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_ipmi_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_ipmi_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_ipmi_manager_avg_busy'] = round((float) (max($zabbix_server_process_ipmi_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_lld_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_lld_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_lld_manager_avg_busy'] = round((float) (max($zabbix_server_process_lld_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_lld_worker_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_lld_worker_avg_busy != []) {
                $zbx_server_internal_processes['process_lld_worker_avg_busy'] = round((float) (max($zabbix_server_process_lld_worker_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_preprocessing_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_preprocessing_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_preprocessing_manager_avg_busy'] = round((float) (max($zabbix_server_process_preprocessing_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_preprocessing_worker_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_preprocessing_worker_avg_busy != []) {
                $zbx_server_internal_processes['process_preprocessing_worker_avg_busy'] = round((float) (max($zabbix_server_process_preprocessing_worker_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_proxy_group_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_proxy_group_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_proxy_group_manager_avg_busy'] = round((float) (max($zabbix_server_process_proxy_group_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_report_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_report_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_report_manager_avg_busy'] = round((float) (max($zabbix_server_process_report_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_report_writer_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_report_writer_avg_busy != []) {
                $zbx_server_internal_processes['process_report_writer_avg_busy'] = round((float) (max($zabbix_server_process_report_writer_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_self_monitoring_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_self_monitoring_avg_busy != []) {
                $zbx_server_internal_processes['process_self-monitoring_avg_busy'] = round((float) (max($zabbix_server_process_self_monitoring_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_service_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_service_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_service_manager_avg_busy'] = round((float) (max($zabbix_server_process_service_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_task_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_task_manager_avg_busy != []) {
                $zbx_server_internal_processes['process_task_manager_avg_busy'] = round((float) (max($zabbix_server_process_task_manager_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_timer_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_timer_avg_busy != []) {
                $zbx_server_internal_processes['process_timer_avg_busy'] = round((float) (max($zabbix_server_process_timer_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_trigger_housekeeper_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_trigger_housekeeper_avg_busy != []) {
                $zbx_server_internal_processes['process_trigger_housekeeper_avg_busy'] = round((float) (max($zabbix_server_process_trigger_housekeeper_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_agent_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_agent_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_agent_poller_avg_busy'] = round((float) (max($zabbix_server_process_agent_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_browser_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_browser_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_browser_poller_avg_busy'] = round((float) (max($zabbix_server_process_browser_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_http_agent_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_http_agent_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_http_agent_poller_avg_busy'] = round((float) (max($zabbix_server_process_http_agent_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_http_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_http_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_http_poller_avg_busy'] = round((float) (max($zabbix_server_process_http_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_icmp_pinger_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_icmp_pinger_avg_busy != []) {
                $zbx_server_collector_processes['process_icmp_pinger_avg_busy'] = round((float) (max($zabbix_server_process_icmp_pinger_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_internal_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_internal_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_internal_poller_avg_busy'] = round((float) (max($zabbix_server_process_internal_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_ipmi_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_ipmi_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_ipmi_poller_avg_busy'] = round((float) (max($zabbix_server_process_ipmi_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_java_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_java_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_java_poller_avg_busy'] = round((float) (max($zabbix_server_process_java_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_odbc_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_odbc_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_odbc_poller_avg_busy'] = round((float) (max($zabbix_server_process_odbc_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_poller_avg_busy'] = round((float) (max($zabbix_server_process_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_proxy_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_proxy_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_proxy_poller_avg_busy'] = round((float) (max($zabbix_server_process_proxy_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_snmp_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_snmp_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_snmp_poller_avg_busy'] = round((float) (max($zabbix_server_process_snmp_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_snmp_trapper_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_snmp_trapper_avg_busy != []) {
                $zbx_server_collector_processes['process_snmp_trapper_avg_busy'] = round((float) (max($zabbix_server_process_snmp_trapper_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_trapper_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_trapper_avg_busy != []) {
                $zbx_server_collector_processes['process_trapper_avg_busy'] = round((float) (max($zabbix_server_process_trapper_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_unreachable_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_unreachable_poller_avg_busy != []) {
                $zbx_server_collector_processes['process_unreachable_poller_avg_busy'] = round((float) (max($zabbix_server_process_unreachable_poller_avg_busy)['value_max'] ?? 0), 2);
            ...
            $zabbix_server_process_vmware_collector_avg_busy = $this->getTrendMaxForCycle(($zabbix_server_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

            if ($zabbix_server_process_vmware_collector_avg_busy != []) {
                $zbx_server_collector_processes['process_vmware_collector_avg_busy'] = round((float) (max($zabbix_server_process_vmware_collector_avg_busy)['value_max'] ?? 0), 2);
            ...
            if ($this->hasInput('zabbix_proxy_ids', [])) {
                ...
                foreach ($zabbix_proxy_ids[0] as $zabbix_proxy_id) {
                    ...
                    $zabbix_proxy_cpu_util = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zbx_proxy_perf['cpu_util'] = round((float) (max($zabbix_proxy_cpu_util)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_cpu_iowait = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zbx_proxy_perf['cpu_iowait'] = round((float) (max($zabbix_proxy_cpu_iowait)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_cpu_load = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zbx_proxy_perf['cpu_load'] = round((float) (max($zabbix_proxy_cpu_load)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_mem_util = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zbx_proxy_perf['mem_util'] = round((float) (max($zabbix_proxy_mem_util)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_swap_util = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));
                    $zbx_proxy_perf['swap_util'] = round(100 - (float) (max($zabbix_proxy_swap_util)['value_max'] ?? 0), 2);
                ...
                foreach ($zabbix_proxy_ids[0] as $zabbix_proxy_id) {
                    ...
                    $zabbix_proxy_rcache_buffer_pused = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_rcache_buffer_pused != []) {
                        $zbx_proxy_cache['rcache_buffer_pused'] = round((float) (max($zabbix_proxy_rcache_buffer_pused)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_wcache_index_pused = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_wcache_index_pused != []) {
                        $zbx_proxy_cache['wcache_index_pused'] = round((float) (max($zabbix_proxy_wcache_index_pused)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_wcache_history_pused = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_wcache_history_pused != []) {
                        $zbx_proxy_cache['wcache_history_pused'] = round((float) (max($zabbix_proxy_wcache_history_pused)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_vmware_buffer_pused = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_vmware_buffer_pused != []) {
                        $zbx_proxy_cache['vmware_buffer_pused'] = round((float) (max($zabbix_proxy_vmware_buffer_pused)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_availability_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_availability_manager_avg_busy != []) {
                        $zbx_proxy_internal_process['process_availability_manager_avg_busy'] = round((float) (max($zabbix_proxy_process_availability_manager_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_configuration_syncer_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_configuration_syncer_avg_busy != []) {
                        $zbx_proxy_internal_process['process_configuration_syncer_avg_busy'] = round((float) (max($zabbix_proxy_process_configuration_syncer_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_data_sender_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_data_sender_avg_busy != []) {
                        $zbx_proxy_internal_process['process_data_sender_avg_busy'] = round((float) (max($zabbix_proxy_process_data_sender_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_discovery_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_discovery_manager_avg_busy != []) {
                        $zbx_proxy_internal_process['process_discovery_manager_avg_busy'] = round((float) (max($zabbix_proxy_process_discovery_manager_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_discovery_worker_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_discovery_worker_avg_busy != []) {
                        $zbx_proxy_internal_process['process_discovery_worker_avg_busy'] = round((float) (max($zabbix_proxy_process_discovery_worker_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_history_syncer_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_history_syncer_avg_busy != []) {
                        $zbx_proxy_internal_process['process_history_syncer_avg_busy'] = round((float) (max($zabbix_proxy_process_history_syncer_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_housekeeper_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_housekeeper_avg_busy != []) {
                        $zbx_proxy_internal_process['process_housekeeper_avg_busy'] = round((float) (max($zabbix_proxy_process_housekeeper_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_ipmi_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_ipmi_manager_avg_busy != []) {
                        $zbx_proxy_internal_process['process_ipmi_manager_avg_busy'] = round((float) (max($zabbix_proxy_process_ipmi_manager_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_preprocessing_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_preprocessing_manager_avg_busy != []) {
                        $zbx_proxy_internal_process['process_preprocessing_manager_avg_busy'] = round((float) (max($zabbix_proxy_process_preprocessing_manager_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_preprocessing_worker_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_preprocessing_worker_avg_busy != []) {
                        $zbx_proxy_internal_process['process_preprocessing_worker_avg_busy'] = round((float) (max($zabbix_proxy_process_preprocessing_worker_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_self_monitoring_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_self_monitoring_avg_busy != []) {
                        $zbx_proxy_internal_process['process_self_monitoring_avg_busy'] = round((float) (max($zabbix_proxy_process_self_monitoring_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_task_manager_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_task_manager_avg_busy != []) {
                        $zbx_proxy_internal_process['process_task_manager_avg_busy'] = round((float) (max($zabbix_proxy_process_task_manager_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_agent_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_agent_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_agent_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_agent_poller_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_browser_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_browser_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_browser_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_browser_poller_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_http_agent_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_http_agent_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_http_agent_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_http_agent_poller_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_http_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_http_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_http_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_http_poller_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_icmp_pinger_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_icmp_pinger_avg_busy != []) {
                        $zbx_proxy_collector_process['process_icmp_pinger_avg_busy'] = round((float) (max($zabbix_proxy_process_icmp_pinger_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_internal_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_internal_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_internal_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_internal_poller_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_ipmi_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_ipmi_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_ipmi_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_ipmi_poller_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_java_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_java_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_java_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_java_poller_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_odbc_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_odbc_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_odbc_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_odbc_poller_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_poller_avg_busy)['value_max'] ?? 0), 2);
                    ...
                    $zabbix_proxy_process_snmp_poller_avg_busy = $this->getTrendMaxForCycle(($zabbix_proxy_itemid[0] ?? 0), (int) $this->getInput('inspection_cycle'));

                    if ($zabbix_proxy_process_snmp_poller_avg_busy != []) {
                        $zbx_proxy_collector_process['process_snmp_poller_avg_busy'] = round((float) (max($zabbix_proxy_process_snmp_poller_avg_busy)['value_max'] ?? 0), 2);
