...
if ($data['host_sel'] == -1) {
    ...
    // Reusable "card" wrapper: a styled panel with an accent header and a body.
    // Inline styles only (no static .css) so it works behind F5/CSP and on both
    // the light and dark Zabbix themes (translucent greys + Claro red accent).
    $inspection_card = function($title, $body) {
        return (new CDiv([
            (new CDiv($title))
                ->addStyle('padding:11px 18px;font-weight:700;font-size:12px;letter-spacing:.05em;'
                    .'text-transform:uppercase;border-left:4px solid #e3001b;'
                    .'background:rgba(127,127,127,.06);border-bottom:1px solid rgba(127,127,127,.15);'),
            (new CDiv($body))
                ->addClass(ZBX_STYLE_CONTAINER)
                ->addStyle('padding:8px 16px 16px;')
        ]))->addStyle('margin:0 0 20px;border-radius:12px;overflow:hidden;'
            .'border:1px solid rgba(127,127,127,.20);box-shadow:0 1px 4px rgba(0,0,0,.12);'
    ...
    $html_page
        ...
        ->addItem($inspection_card(_('Analysis of inspection results'), ${"info_table_" . strval($n)}));
    ...
    $html_page
        ->addItem($inspection_card(_('System information'), ${"info_table_" . strval($n)}));

    $n++;

    ${"info_table_" . strval($n)} = (new CTableInfo())
        ->setHeader([(new CColHeader(_('Hostname')))->addStyle('width: 8%'), _('Uname'), _('IP'), _('Version'), _('CPU num'), _('Mem size'), _('Runtime')])
        ->setHeadingColumn(0)
        ->addClass(ZBX_STYLE_LIST_TABLE_STICKY_HEADER);
    ...
    if (count($data['zabbix_server_info']) > 0) {
        foreach ($data['zabbix_server_info'] as $zabbix_server_info) {
            ${"info_table_" . strval($n)}->addRow([
                ...
                ($zabbix_server_info['zabbix_server_runtime'] === 'docker'
                    ? (new CSpan(_('Docker')))->addStyle('color:#0db7ed;font-weight:bold;')
                    : (new CSpan(_('Bare-metal')))->addStyle('color:#768d99;'))
            ]);
        }
    }
    ...
    if (count($data['zabbix_proxy_info']) > 0) {
        foreach ($data['zabbix_proxy_info'] as $zabbix_proxy_info) {
            ${"info_table_" . strval($n)}->addRow([
                ...
                ($zabbix_proxy_info['zabbix_proxy_runtime'] === 'docker'
                    ? (new CSpan(_('Docker')))->addStyle('color:#0db7ed;font-weight:bold;')
                    : (new CSpan(_('Bare-metal')))->addStyle('color:#768d99;'))
            ]);
        }
    ...
    if (count($data['zabbix_database_info']) > 0) {
        foreach ($data['zabbix_database_info'] as $zabbix_database_info) {
            ${"info_table_" . strval($n)}->addRow([
                ...
                ($zabbix_database_info['zabbix_database_runtime'] === 'docker'
                    ? (new CSpan(_('Docker')))->addStyle('color:#0db7ed;font-weight:bold;')
                    : (new CSpan(_('Bare-metal')))->addStyle('color:#768d99;'))
            ]);
        }
    ...
    if ((count($data['zabbix_server_info']) > 0) || (count($data['zabbix_proxy_info']) > 0) || (count($data['zabbix_database_info']) > 0)) {
        $html_page
            ->addItem($inspection_card(_('Server overview'), ${"info_table_" . strval($n)}));
    }
    ...
    if (count($data['zabbix_proxy_info']) > 0) {
        ...
        $html_page
            ->addItem($inspection_card(_('Proxy information'), ${"info_table_" . strval($n)}));
    }
    ...
    if (count($data['zabbix_database_info']) > 0) {
        ...
        $html_page
            ->addItem($inspection_card(_('Database information'), ${"info_table_" . strval($n)}));
    }
    ...
    $html_page
        ->addItem($inspection_card(_('Queue overview'), ${"info_table_" . strval($n)}));
    ...
    $html_page
        ->addItem($inspection_card(_('Queue detail'), ${"info_table_" . strval($n)}));
    ...
    $html_page
        ->addItem($inspection_card(_('Problem overview'), ${"info_table_" . strval($n)}));
    ...
    $html_page
        ->addItem($inspection_card(_('Server performance metric'), ${"info_table_" . strval($n)}));
    ...
    $html_page
        ->addItem($inspection_card(_('Server cache metric'), ${"info_table_" . strval($n)}));
    ...
    $html_page
        ->addItem($inspection_card(_('Server internal processes metric'), ${"info_table_" . strval($n)}));
    ...
    $html_page
        ->addItem($inspection_card(_('Server data collector processes metric'), ${"info_table_" . strval($n)}));
    ...
    if (count($data['zabbix_proxy_info']) > 0) {
        ...
        $html_page
            ->addItem($inspection_card(_('Proxy performance metric'), ${"info_table_" . strval($n)}));
        ...
        $html_page
            ->addItem($inspection_card(_('Proxy cache metric'), ${"info_table_" . strval($n)}));
        ...
        $html_page
            ->addItem($inspection_card(_('Proxy internal process metric'), ${"info_table_" . strval($n)}));
        ...
        $html_page
            ->addItem($inspection_card(_('Proxy collector process metric'), ${"info_table_" . strval($n)}));
    ...
    if (count($data['zabbix_server_info']) > 0) {
        foreach ($data['zabbix_server_info'] as $zabbix_server_info) {
            ${"info_table_" . strval($n)}->addRow([
                $zabbix_server_info['zabbix_server_hostname'],
                ($zabbix_server_info['zabbix_server_runtime'] === 'docker' && trim($zabbix_server_info['zabbix_server_log']) === ''
                    ? (new CSpan(_('Logs go to the container stdout - use "docker logs <container>".')))->addStyle('color:#768d99;font-style:italic;')
                    : new CPre([$zabbix_server_info['zabbix_server_log'], BR()]))
            ]);
        }
    }
    ...
    if (count($data['zabbix_proxy_info']) > 0) {
        foreach ($data['zabbix_proxy_info'] as $zabbix_proxy_info) {
            ${"info_table_" . strval($n)}->addRow([
                $zabbix_proxy_info['zabbix_proxy_hostname'],
                ($zabbix_proxy_info['zabbix_proxy_runtime'] === 'docker' && trim($zabbix_proxy_info['zabbix_proxy_log']) === ''
                    ? (new CSpan(_('Logs go to the container stdout - use "docker logs <container>".')))->addStyle('color:#768d99;font-style:italic;')
                    : new CPre([$zabbix_proxy_info['zabbix_proxy_log'], BR()]))
            ]);
        }
    }

    $html_page
        ->addItem($inspection_card(_('Log error information'), ${"info_table_" . strval($n)}));
    ...
    if (count($data['zabbix_server_info']) > 0) {
        foreach ($data['zabbix_server_info'] as $zabbix_server_info) {
            ${"info_table_" . strval($n)}->addRow([
                $zabbix_server_info['zabbix_server_hostname'],
                [
                    (new CSpan(_('Source: ') . ($zabbix_server_info['zabbix_server_conf_source'] === 'env' ? _('container environment (ZBX_*)') : _('file (zabbix_server.conf)'))))->addStyle('color:#768d99;font-style:italic;'),
                    BR(),
                    new CPre([$zabbix_server_info['zabbix_server_conf'], BR()])
                ]
            ]);
        }
    }
    ...
    if (count($data['zabbix_proxy_info']) > 0) {
        foreach ($data['zabbix_proxy_info'] as $zabbix_proxy_info) {
            ${"info_table_" . strval($n)}->addRow([
                $zabbix_proxy_info['zabbix_proxy_hostname'],
                [
                    (new CSpan(_('Source: ') . ($zabbix_proxy_info['zabbix_proxy_conf_source'] === 'env' ? _('container environment (ZBX_*)') : _('file (zabbix_proxy.conf)'))))->addStyle('color:#768d99;font-style:italic;'),
                    BR(),
                    new CPre([$zabbix_proxy_info['zabbix_proxy_conf'], BR()])
                ]
            ]);
        }
    }
